...
The browser proof/readback now records a `viewport_3d_readback` artifact with canvas marker `studio-3d-webgl-canvas`, visible renderable count, selected target, preview ghost, applied renderable, and explicit non-claims. Task `asha#3043` adds a deterministic `viewport_camera_tool_interaction_proof` nested in the scene-view/readback: the shared timeline records a GUI frame-selected command (`inspection.editor_state`), an agent screen-point selection command, and a GUI preview-ghost command, while camera/tool before/after hashes and stale-readback guards fail closed if camera, selection, or preview evidence diverges. Task `asha#3044` adds `viewport_pick_hit_test_evidence`: the readback records the selected-target screen point, viewport dimensions/hash, camera pose/projection hash, hit renderable/voxel/face/normal/world point, a structured background no-hit result, selection/hierarchy/timeline cross-checks, and stale guards that fail closed if camera or viewport evidence changes without refreshed pick evidence. This is still browser projection evidence only: it does not claim native/WASM runtime bridge execution, Agora compositor capture, hardware GPU evidence, or performance.
