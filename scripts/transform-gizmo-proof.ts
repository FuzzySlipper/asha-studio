...
function main(): void {
  ...
  const steps: StepProof[] = [
    ...
    { id: 6, requirement: 'apply command result carries the typed before/after translation evidence', status: (() => { const output = applyOutput; return output !== null && 'translationAfter' in output && output.entityId === gizmo.selectedEntityId && output.renderableId === gizmo.selectedEntityId && output.applied === true && output.axis === gizmo.activeAxis && output.transformHash === gizmo.edit.transformHash && output.translationAfter.join(',') === gizmo.transform.after.join(',') ? 'passed' : 'failed'; })(), evidence: JSON.stringify(applyOutput) },
    ...
    { id: 10, requirement: 'negative smokes fail closed for stale/missing-handle/mismatch/private-path/missing-entity', status: gizmo.negativeSmokes.length === 5 && gizmo.negativeSmokes.every((smoke) => smoke.actualOutcome === 'failed_closed' && smoke.diagnosticCodes.includes(smoke.code)) ? 'passed' : 'failed', evidence: gizmo.negativeSmokes.map((smoke) => `${smoke.code}=${smoke.actualOutcome}`).join('; ') },
