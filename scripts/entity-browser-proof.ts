...
function main(): void {
  ...
  const steps: StepProof[] = [
    ...
    { id: 10, requirement: 'selection.set_active_entity command result matches its public output schema and carries the selected entity', status: (() => { const result = workspace.commandResults.find((entry) => entry.commandId === 'selection.set_active_entity'); const output = result?.output; return output !== null && output !== undefined && 'selected' in output && output.entityId === browser.selection.selectedEntityId && output.renderableId === browser.selection.viewportRenderableId && output.selected === true ? 'passed' : 'failed'; })(), evidence: JSON.stringify(workspace.commandResults.find((entry) => entry.commandId === 'selection.set_active_entity')?.output ?? null) },
  ];
