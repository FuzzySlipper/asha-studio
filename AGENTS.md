...
- Model/material preview logic lives in `src/model-material-preview.ts`; it may use `@asha/contracts` package-root DTOs for catalog/material/static-mesh/render-diff metadata, but it must not pretend Studio has first-class ASHA model/material command/runtime verbs until those are promoted upstream.
...
Task `asha#2740` reconciles V1 docs and known limitations after the Studio proof series. Current V1 reality: task `asha#2737` added bounded command batch transaction metadata and typed V1 voxel revert evidence; task `asha#2736` added the public-contract model/material preview artifact and records missing first-class model/material command/runtime surfaces as upstream follow-up work; task `asha#2739` added `pnpm run proof:browser` for Chromium browser screenshot capture/readback of the Studio app and V1 proof routes; task `asha#2738` added the end-to-end `pnpm run proof:v1` visual edit proof command and generated artifact readback; task `asha#2735` added software visual evidence and fail-closed review artifact export; task `asha#2734` added the visible voxel inspect/select/preview/apply workflow; task `asha#2733` added the session/workspace model and shared command timeline; task `asha#2732` added compatibility metadata/readback and fail-closed checks; task `asha#2731` hardened the boundary policy/checker; task `asha#2730` created the real frontend shell baseline. Later tasks own native runtime bridge integration, first-class model/material command surfaces, Agora compositor capture as a formal proof command, hardware render evidence, durable timeline persistence, and richer visual domains.
