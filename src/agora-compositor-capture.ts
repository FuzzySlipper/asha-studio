...
function buildNegativeSmokes(evidence: AgoraCaptureEvidence): readonly AgoraCaptureNegativeSmoke[] {
  ...
  smokes.push({ id: 'negative:stale-compositor-frame', code: 'stale_compositor_frame', scenario: 'The capture predates this launch timestamp.', expectedOutcome: 'failed_closed', actualOutcome: staleDiags.some((d) => d.code === 'stale_compositor_frame') ? 'failed_closed' : 'ready', diagnosticCodes: staleDiags.map((d) => d.code) });
